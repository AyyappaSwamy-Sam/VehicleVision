...
def draw_box_label(img, bbox_cv2, box_color=(0, 255, 255), show_label=True):
    ...
    cv.rectangle(img, (left, top), (right, bottom), box_color, 4)
    
    if show_label:
        # Draw a filled box on top of the bounding box (as the background for the labels)
        cv.rectangle(img, (left-2, top-45), (right+2, top), box_color, -1, 1)
        
        # Output the labels that show the x and y coordinates of the bounding box center.
        text_x= 'x='+str((left+right)/2)
        cv.putText(img,text_x,(left,top-25), font, font_size, font_color, 1, cv.LINE_AA)
        text_y= 'y='+str((top+bottom)/2)
        cv.putText(img,text_y,(left,top-5), font, font_size, font_color, 1, cv.LINE_AA)
